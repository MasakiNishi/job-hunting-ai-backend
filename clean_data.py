...
def filter_arrangement(title, location, desc):
    ...
    # We want prioritize iterating through titles, then descriptions
    ...
    # Next, check if any locations are "anywhere"
    for word in remote_location:
...
def filter_experience(title, description, job_highlights):
    ...
    for pattern in senior_level:
        if re.search(pattern, title_lower):
            return 'Senior-Level'
    for pattern in entry_level:
        if re.search(pattern, title_lower):
            return 'Entry-Level'
    for pattern in junior_level:
        if re.search(pattern, title_lower):
            return 'Junior-Level'
    for pattern in int_level:
        if re.search(pattern, title_lower):
